...
def sortfunc(m):
	...
	Sort by individual interrogation time:
	Assume that expected time is [time] * [denominator] / [nominator], i.e.
	as if prob is 1/5, we have to interrogate 5 times.
	"""
	return m[1][0] / (float(m[1][1]) / float(m[1][2]))
	

def answer(minions):
  """
  Try naive, assume that just sorting by individual interrogation time is enough.
  """
  l = len(minions)
  minions_and_indices = zip(range(l), minions)
    
  sorted_minions = sorted(minions_and_indices, key=lambda m: sortfunc(m))
  retval = [m[0] for m in sorted_minions]
